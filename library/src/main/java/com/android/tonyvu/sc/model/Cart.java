...
public class Cart implements Serializable {
    ...
    public void remove(final Saleable sellable, int quantity) throws ProductNotFoundException, QuantityOutOfRangeException {
        ...
        if (quantity < 0 || quantity > productQuantity)
            throw new QuantityOutOfRangeException(quantity + " 유효한 수량이 아닙니다. 이는 음수가 아니며 장바구니에있는 제품의 현재 수량보다 작아야합니다");
    ...
    public String toString() {
        ...
        for (Entry<Saleable, Integer> entry : cartItemMap.entrySet()) {
            strBuilder.append(String.format(Locale.KOREA,"상품: %s, 가격: %f, 수량: %d%n", entry.getKey().getName(), entry.getKey().getPrice(), entry.getValue()));
        }
        strBuilder.append(String.format(Locale.KOREA,"총 수량: %d, 총 가격: %f", totalQuantity, totalPrice));
